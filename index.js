...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6283820776095@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"6283820776095@s.whatsapp.net"]
			const adminbotnumber = ["6283820776095@s.whatsapp.net"]
			const frendsowner = ["6283820776095@s.whatsapp.net"]
			const premium = ["6283820776095@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isPublic) return reply(mess.only.public)
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT Zky ✨* 」\n*│+wa.me/6283820776095*╰──────*「 Zky BOT」*────*\n\n*_JIKA INGIN MENJADI ADMIN  Zky BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/Gv5W97v/IMG-20210106-WA1995.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6283820776095*╰──────*「 Zky BOT 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  Zky BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
            ...
            case 'bodoh':
				...
				if (!isOwner) return reply('Kamu Siapa Owner?')
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`REPLY GAMBAR JIKA INGIN MEMATIKAN BOT!`)
					}
					break
			case 'creator':
			case 'owner':
...
if (isBanned) return reply(mess.only.benned)    
	...
	if (!isPublic) return reply(mess.only.public)
				...
				if (!isPublic) return reply(mess.only.public)
					hobby = body.slice(1)
					const hob =['Memasak','Membantu Atok','Mabar','Nobar','Sosmedtan','Membantu Orang lain','Nonton Anime','Nonton Drakor','Naik Motor','Nyanyi','Menari','Bertumbuk','Menggambar','Foto fotoan Ga jelas','Maen Game','Berbicara Sendiri','Coli','Ngentod','Rebahan']
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*Gopay :* _0895412849912_\n*Saweria :* _https://saweria.co/darkbot2412\n*Dana :* _085156492564_', text, { quoted: mek })
					break
                case 'ttp':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('6283820776095@s.whatsapp.net', options, text, {quoted: mek})
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
			...
			if (!isPublic) return reply(mess.only.public)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  Zky BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
